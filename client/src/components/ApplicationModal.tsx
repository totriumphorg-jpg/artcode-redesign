...
export const ApplicationModal: React.FC<ApplicationModalProps> = ({
  ...
  useEffect(() => {
    if (isOpen) setSelectedComp(defaultCompetitionId);
  ...
  return (
    <Dialog open={isOpen} onOpenChange={(open) => !open && resetAndClose()}>
      <DialogContent className="w-[calc(100vw-1rem)] max-w-[calc(100vw-1rem)] sm:!max-w-2xl min-w-0 bg-[#0f172a] border-slate-800 text-slate-100 max-h-[88vh] overflow-x-hidden overflow-y-auto overscroll-contain p-4 sm:p-6 rounded-2xl">
        {!submitted ? (
          <>
            <DialogHeader className="pr-8">
              ...
              <DialogTitle className="text-xl sm:text-2xl font-serif text-white leading-tight">Заявка на участие в конкурсе</DialogTitle>
              <DialogDescription className="text-slate-300 text-xs sm:text-sm leading-relaxed">Заполните данные участника или коллектива и прикрепите ссылку на конкурсный материал: Ютуб, Рутуб, ВК Видео или облачное хранилище.</DialogDescription>
            </DialogHeader>

            <form onSubmit={handleSubmit} className="min-w-0 space-y-3.5 mt-2 pb-1">
              <div className="space-y-1.5">
                ...
                <Select value={selectedComp} onValueChange={setSelectedComp}>
                  <SelectTrigger id="comp-select" className="!w-full !max-w-full min-w-0 bg-[#020617] border-slate-700 text-white text-[16px] sm:text-sm [&>span]:truncate"><SelectValue placeholder="Выберите конкурс" /></SelectTrigger>
                ...
                <div className="p-2.5 bg-[#020617] rounded-lg border border-slate-800 text-xs text-slate-300 flex flex-col sm:flex-row gap-1 sm:items-center sm:justify-between"><span>Оплата: <strong className="text-amber-400 font-medium">карты российских банков и МИР</strong></span><span className="text-slate-400 sm:text-right">{comp.resultsDate}</span></div>
              </div>

              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                <div className="space-y-1.5"><Label htmlFor="fullname" className="text-xs text-slate-300">ФИО участника или название коллектива <span className="text-amber-400">*</span></Label><Input id="fullname" required placeholder="Например: Рыбченков Тимофей" value={fullName} onChange={(e) => setFullName(e.target.value)} className="bg-[#020617] border-slate-700 text-white text-[16px] sm:text-sm" /></div>
                <div className="space-y-1.5"><Label htmlFor="city" className="text-xs text-slate-300">Город / страна <span className="text-amber-400">*</span></Label><Input id="city" required placeholder="Например: Санкт-Петербург, Россия" value={city} onChange={(e) => setCity(e.target.value)} className="bg-[#020617] border-slate-700 text-white text-[16px] sm:text-sm" /></div>
              </div>

              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                <div className="space-y-1.5"><Label htmlFor="email" className="text-xs text-slate-300">Электронная почта для наградного пакета <span className="text-amber-400">*</span></Label><Input id="email" type="email" required placeholder="artist@example.com" value={email} onChange={(e) => setEmail(e.target.value)} className="bg-[#020617] border-slate-700 text-white text-[16px] sm:text-sm" /></div>
                <div className="space-y-1.5"><Label htmlFor="phone" className="text-xs text-slate-300">Телефон / контакт <span className="text-amber-400">*</span></Label><Input id="phone" required placeholder="+7 (999) 000-00-00" value={phone} onChange={(e) => setPhone(e.target.value)} className="bg-[#020617] border-slate-700 text-white text-[16px] sm:text-sm" /></div>
              </div>

              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                <div className="space-y-1.5"><Label htmlFor="age-cat" className="text-xs text-slate-300">Возрастная категория</Label><Select value={ageCategory} onValueChange={setAgeCategory}><SelectTrigger id="age-cat" className="bg-[#020617] border-slate-700 text-white text-[16px] sm:text-sm"><SelectValue placeholder="Категория" /></SelectTrigger><SelectContent className="bg-[#0f172a] border-slate-700 text-white"><SelectItem value="under-6">До 6 лет</SelectItem><SelectItem value="7-9">7 – 9 лет</SelectItem><SelectItem value="10-12">10 – 12 лет</SelectItem><SelectItem value="13-15">13 – 15 лет</SelectItem><SelectItem value="16-18">16 – 18 лет</SelectItem><SelectItem value="19-25">19 – 25 лет</SelectItem><SelectItem value="25-plus">Старше 25 лет</SelectItem><SelectItem value="mixed">Смешанная категория</SelectItem></SelectContent></Select></div>
                <div className="space-y-1.5"><Label htmlFor="nomination" className="text-xs text-slate-300">Номинация / произведение <span className="text-amber-400">*</span></Label><Input id="nomination" required placeholder="Например: Народный вокал / Романс" value={nomination} onChange={(e) => setNomination(e.target.value)} className="bg-[#020617] border-slate-700 text-white text-[16px] sm:text-sm" /></div>
              </div>

              <div className="space-y-1.5"><Label htmlFor="institution" className="text-xs text-slate-300">Учебное заведение / ФИО руководителя или педагога</Label><Input id="institution" placeholder="Например: ДМШ №1, рук. Иванова М.П." value={institution} onChange={(e) => setInstitution(e.target.value)} className="bg-[#020617] border-slate-700 text-white text-[16px] sm:text-sm" /></div>

              <div className="space-y-1.5"><Label htmlFor="media" className="text-xs text-slate-300 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-0.5"><span>Ссылка на видеозапись или материалы <span className="text-amber-400">*</span></span><span className="text-[11px] text-slate-400">Ютуб, Рутуб, ВК или Яндекс.Диск</span></Label><div className="relative"><UploadCloud className="absolute left-3 top-3 w-4 h-4 text-slate-400" /><Input id="media" required placeholder="https://..." value={mediaLink} onChange={(e) => setMediaLink(e.target.value)} className="pl-9 bg-[#020617] border-slate-700 text-white text-[16px] sm:text-sm" /></div><p className="text-[11px] text-slate-400">Съемка живого выступления на статичную камеру, качество не менее 720р, запись не старше 2 лет.</p></div>
